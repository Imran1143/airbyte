...
/**
 * Custom Micronaut {@link PropertySource} that reads the command line arguments
 * provided via the connector CLI and turns them into configuration properties.
 * This allows the arguments to be injected into code that depends on them via Micronaut.
 * <p />
 * This property source adds the following properties to the configuration if the matching value is present
 * in the CLI arguments:
 * <ul>
 * <li><b>airbyte.connector.operation</b> - the operation argument (e.g. "check", "discover", etc)</li>
 * <li><b>airbyte.connector.catalog</b> - the Airbyte configured catalog as JSON read from the configured catalog file path argument, if present.</li>
 * <li><b>airbyte.connector.config</b> - the normalized Airbyte connector configuration read from the configured connector configuration file path argument, if present.</li>
 * <li><b>airbyte.connector.state</b> - the Airbyte connector state as JSON read from the state file path argument, if present.</li>
 * </ol>
 */
class ConnectorConfigurationPropertySource(commandLine: CommandLine) :
    ...
    companion object {
        ...
        const val CONNECTOR_CATALOG_KEY: String = "configured"
