...
class DetectStreamToFlush
    ...
    internal constructor(
         ...
         * each individual queue isn't that large.
        ...
        fun computeQueueThreshold(): Long {
            val isBuffer90Full =
                EAGER_FLUSH_THRESHOLD <= bufferDequeue.totalGlobalQueueSizeBytes.toDouble() / bufferDequeue.maxQueueSizeBytes
         ...
         * [.isSizeTriggered] and [.isTimeTriggered] for details
         * on these triggers.
         *
            ...
            for (stream in orderStreamsByPriority(bufferDequeue.bufferedStreams)) {
                val latestFlushTimeMs =
                    latestFlushTimeMsPerStream.computeIfAbsent(
                        stream,
                    ) { _: StreamDescriptor -> nowProvider.millis() }
        ...
        fun estimateSizeOfRunningWorkers(
            ...
            val workersWithoutBatchesCount =
                runningWorkerBatchesSizes.stream().filter { obj: Optional<Long> -> obj.isEmpty }
                    .count()
